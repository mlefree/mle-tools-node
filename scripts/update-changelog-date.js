...
fs.readFile(changelogPath, 'utf8', (err, data) => {
    ...
    // We need to get the current version from package.json to ensure we're updating the correct entry
    const packageJson = require('../package.json');
    const currentVersion = packageJson.version;
    console.log(`Current version from package.json: ${currentVersion}`);

    // Create a regex pattern that specifically looks for the current version
    const versionPattern = new RegExp(`## \\[${currentVersion}\\] - \\d{4}-\\d{2}-\\d{2}`);
